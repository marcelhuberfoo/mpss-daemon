...
/*
 * This is for compatibility with external applications compiled with C that links with libmpssconfig
 */
...
**/
EXTERN_C void boot_linux(int node);
...
**/
EXTERN_C void boot_media(int node, const char *media_path);
...
**/
EXTERN_C char *get_state(int node);
 ...
 * reset node
**/
EXTERN_C void reset_node(int node);
...
#ifndef DEFAULT_BZIMAGE_KNC
#define DEFAULT_BZIMAGE_KNC "bzImage-knightscorner"
#endif

#ifndef DEFAULT_SYSMAP_KNC
#define DEFAULT_SYSMAP_KNC "System.map-knightscorner"
#endif

#ifndef DEFAULT_INITRD_KNC
#define DEFAULT_INITRD_KNC "initramfs-knightscorner.cpio.gz"
#endif
...
/**
 * mpssenv_clean - free all the memory associated with the the mpssenv_init.
 *     \param menv	Initialized MIC environment
*/
void mpssenv_clean(struct mpss_env *menv);
...
const char *mpss_version_to_str(int mpss_version);

enum mfamily {	// Family parameter
	MIC_FAMILY_UNKNOWN_VALUE = 0,
	MIC_FAMILY_KNC_VALUE,
	MIC_FAMILY_KNL_VALUE,
	MIC_FAMILY_VALUES	// number of possible Family values
};

enum mmpss_version { // MPSSVersion parameter
	MPSS_VERSION_UNKNOWN_VALUE = 0,
	MPSS_VERSION_3_VALUE,
	MPSS_VERSION_4_VALUE,
	MPSS_VERSION_VALUES	// number of possible MPSSVersion values
};
...
struct mconfig {	// Contains all config data produced by parser
	...
	enum   mfamily		family;
	enum   mmpss_version	mpss_version;
};
...
*/
struct mbridge *mpss_insert_bridge(char *name, unsigned int type, char *ip,
 ...
 *     \param ip	MIC card IP address passed to the kernel cmdline
 *
